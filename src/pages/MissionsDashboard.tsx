...
const MissionsDashboard = () => {
  ...
  const fetchMisiones = async () => {
    // Fetch root missions (no parent)
    const { data: allMisiones, error } = await supabase
      .from("misiones")
      .select("*")
      .is("mission_parent_id", null)
    ...
    const enriched: MisionWithCounts[] = await Promise.all(
      (allMisiones || []).map(async (m: any) => {
        ...
        return {
          ...m,
          tags: m.tags || [],
          submisionCount: subCount || 0,
          encounterCount: encCount || 0,
        };
      })
    );
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          {/* Context panel - hidden on mobile by default */}
          ...
          <div className="lg:col-span-3 order-1 lg:order-2">
            ...
            {loading ? (
              ...
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="text-center py-16 ornate-border rounded-lg parchment-bg"
              >
              ...
              <div className="space-y-3">
                {filtered.map((mision, i) => (
                  <motion.div
                    ...
                    className="ornate-border rounded-lg parchment-bg overflow-hidden"
                  >
                    {/* Card header - tappable */}
                    <button
                      onClick={() => setExpandedId(expandedId === mision.id ? null : mision.id)}
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        {expandedId === mision.id ? (
                          <ChevronDown size={20} className="text-muted-foreground shrink-0 mt-1" />
                        ) : (
                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-3">
                        ...
                        {mision.submisionCount > 0 && (
                          <span className="text-xs text-muted-foreground flex items-center gap-1">
                            <Target size={12} /> {mision.submisionCount} sub
                          </span>
                        )}
                        {mision.encounterCount > 0 && (
                          <span className="text-xs text-muted-foreground flex items-center gap-1">
                            <Swords size={12} /> {mision.encounterCount}
                          </span>
                        )}
                      </div>
                    </button>
                    ...
                    <AnimatePresence>
                      {expandedId === mision.id && (
                        ...
                        >
                          <div className="px-4 pb-4 sm:px-5 sm:pb-5 border-t border-border/50 pt-4 space-y-3">
                            ...
                            <button
                              onClick={() => navigate(`/mission/${mision.id}`)}
